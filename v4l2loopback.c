...
#define V4L2LOOPBACK_VERSION_CODE                  \
	KERNEL_VERSION(V4L2LOOPBACK_VERSION_MAJOR, \
...
#define acquire_token(dev, opener, label, token)                                 \
	do {                                                                     \
		...
		dprintk("acquire_token: opener=%p got token=0x%x "               \
			"(dynamic=%d)\n",                                        \
			opener,                                                  \
			token,                                                   \
...
#define release_token(dev, opener, label)                                                 \
	do {                                                                              \
		...
		if (releasing_token) {                                                    \
			...
			dprintk("release_token: opener=%p released "                      \
				"token=0x%x (dynamic=%d)\n",                              \
				opener,                                                   \
				releasing_token,                                          \
				(dev)->use_dynamic_buffering);                            \
		}                                                                         \
...
static int resize_dynamic_buffer(struct v4l2_loopback_device *dev,
				 u32 new_capacity);
static int
dynamic_buffer_write(struct v4l2_loopback_device *dev, const u8 *src, u32 len);
...
static int __resize_buffer_locked(struct dynamic_buffer *dbuf,
				  u32 new_capacity);
static void __copy_circular_data(const void *src_data,
				 u32 src_size,
				 u32 src_pos,
				 void *dst_data,
				 u32 copy_len);
...
static int v4l2l_fill_format(struct v4l2_format *fmt,
			     const u32 minwidth,
			     const u32 maxwidth,
			     const u32 minheight,
			     const u32 maxheight)
{
	...
	} else if (!v4l2_fill_pixfmt(&fmt0.fmt.pix,
				     pixelformat,
				     width,
				     height)) {
		;
	} else if (!v4l2_fill_pixfmt_mp(&fmt0.fmt.pix_mp,
					pixelformat,
					width,
					height)) {
		;
...
static int pix_format_eq(const struct v4l2_pix_format *ref,
			 const struct v4l2_pix_format *tgt,
			 int strict)
{
			     ...
			     struct v4l2_fract *tpf)
{
	...
	dev->frame_jiffies = max(1UL,
				 (msecs_to_jiffies(1000) * tpf->numerator) /
					 tpf->denominator);
}
...
static ssize_t
attr_show_format(struct device *cd, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(format,
		   S_IRUGO | S_IWUSR,
		   attr_show_format,
		   attr_store_format);

static ssize_t
attr_show_buffers(struct device *cd, struct device_attribute *attr, char *buf)
{
	struct v4l2_loopback_device *dev = v4l2loopback_cd2dev(cd);
...
static DEVICE_ATTR(max_openers,
		   S_IRUGO | S_IWUSR,
		   attr_show_maxopeners,
		   attr_store_maxopeners);

static ssize_t
attr_show_state(struct device *cd, struct device_attribute *attr, char *buf)
{
	struct v4l2_loopback_device *dev = v4l2loopback_cd2dev(cd);
 ...
 */
static DEVICE_ATTR(dynamic_buffering,
		   S_IRUGO | S_IWUSR,
		   attr_show_dynamic_buffering,
		   attr_store_dynamic_buffering);
					 ...
					 char *buf)
{
	...
	format_buffer_size(dbuf->stats.total_bytes_written,
			   size_str,
			   sizeof(size_str));
	len += sprintf(buf + len, "Total written:   %s\n", size_str);
	format_buffer_size(dbuf->stats.total_bytes_read,
			   size_str,
			   sizeof(size_str));
	...
	len += sprintf(buf + len,
		       "Expand count:    %u (avg time: %u µs)\n",
		       dbuf->stats.expand_count,
		       avg_expand_time_us);
	len += sprintf(buf + len,
		       "Shrink count:    %u (avg time: %u µs)\n",
		       dbuf->stats.shrink_count,
		       avg_shrink_time_us);
	format_buffer_size(dbuf->stats.max_capacity_reached,
			   size_str,
			   sizeof(size_str));
...
static DEVICE_ATTR(dynamic_buffer_reset_stats,
		   0200,
		   NULL,
			       ...
			       struct v4l2_loopback_device **device)
{
	...
	int err = idr_for_each(&v4l2loopback_index_idr,
			       &v4l2loopback_lookup_cb,
			       &data);
	if (1 == err) {
		if (device)
			*device = data.device;
...
static int allocate_buffers(struct v4l2_loopback_device *dev,
			    struct v4l2_pix_format *pix_format);
static void
init_buffers(struct v4l2_loopback_device *dev, u32 bytes_used, u32 buffer_size);
 ...
 */
static int
vidioc_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
...
static int
vidioc_enum_fmt_vid(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	struct v4l2_loopback_opener *opener = fh_to_opener(fh);
 ...
 */
static int
vidioc_try_fmt_vid(struct file *file, void *fh, struct v4l2_format *f)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	struct v4l2_loopback_opener *opener = fh_to_opener(fh);
...
static int vidioc_s_fmt_vid(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (opener->buffer_count > 0) {
		dprintk("S_FMT: buffers already allocated, requiring "
			"release\n");
		return -EBUSY;
	}
	...
	if (dynamic_mode) {
		...
		dprintk("S_FMT: dynamic mode - allowing format without token "
			"restriction\n");
	} else {
		/* Static mode: traditional validation */
		if (!(dev->format_tokens & token)) {
			result = -EBUSY;
			dprintk("S_FMT: token %u not available in static "
				"mode\n",
				token);
	...
	if (changed || has_no_owners(dev) || dynamic_mode) {
		...
		if (result < 0) {
			pr_err("v4l2-loopback: buffer allocation failure "
			       "(%d)\n",
			       result);
			goto exit_s_fmt_unlock;
		}
		...
		if (dynamic_mode && !dev->dbuf) {
			...
			if (result < 0) {
				pr_err("v4l2-loopback: dynamic buffer "
	...
	if ((dev->timeout_image && changed) ||
		...
		if (result < 0) {
			pr_err("v4l2-loopback: timeout buffer allocation "
			       "failed (%d)\n",
			       result);
			goto exit_s_fmt_free;
		}
...
static int
vidioc_enum_fmt_cap(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
...
static int
vidioc_try_fmt_cap(struct file *file, void *fh, struct v4l2_format *f)
{
...
static int
vidioc_enum_fmt_out(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	return vidioc_enum_fmt_vid(file, fh, f);
}
...
static int
vidioc_try_fmt_out(struct file *file, void *fh, struct v4l2_format *f)
{
	return vidioc_try_fmt_vid(file, fh, f);
}
...
static int
vidioc_g_fmt_overlay(struct file *file, void *priv, struct v4l2_format *fmt)
{
	return 0;
}

static int
vidioc_s_fmt_overlay(struct file *file, void *priv, struct v4l2_format *fmt)
{
	return 0;
}
#endif /* V4L2L_OVERLAY */
...
static int
v4l2loopback_set_ctrl(struct v4l2_loopback_device *dev, u32 id, s64 val)
{
	int result = 0;
	switch (id) {
 ...
 */
static int
vidioc_enum_output(struct file *file, void *fh, struct v4l2_output *outp)
{
 ...
 */
static int
vidioc_enum_input(struct file *file, void *fh, struct v4l2_input *inp)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	struct v4l2_loopback_opener *opener = fh_to_opener(fh);
	__u32 index = inp->index;

	dprintk("vidioc_enum_input: stream_tokens=0x%x, keep_format=%d, "
		"dynamic=%d\n",
		dev->stream_tokens,
		dev->keep_format,
		dev->use_dynamic_buffering);
...
/* forward declaration */
static int
vidioc_streamoff(struct file *file, void *fh, enum v4l2_buf_type type);
/* negotiate buffer type
 * only mmap streaming supported
 * called on VIDIOC_REQBUFS
 */
static int
vidioc_reqbufs(struct file *file, void *fh, struct v4l2_requestbuffers *reqbuf)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	struct v4l2_loopback_opener *opener = fh_to_opener(fh);
	u32 token = opener->io_method == V4L2L_IO_TIMEOUT ?
...
static int vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		...
		if (dev->use_dynamic_buffering && dev->dbuf) {
			...
			if (bytes_to_write > 0) {
				int ret = dynamic_buffer_write(dev,
							       src,
							       bytes_to_write);
				if (ret < 0) {
					dprintk("Failed to write to dynamic "
						"buffer: %d\n",
						ret);
				} else {
					dprintk("Successfully wrote %d bytes "
						"to dynamic buffer\n",
						ret);
				}
			}
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	dprintk("STREAMON START: type=%d, token=0x%x, dynamic=%d, "
		"stream_tokens=0x%x\n",
		type,
		token,
		dynamic_mode,
	...
	if (dev->use_dynamic_buffering && !dev->dbuf &&
		...
		mutex_unlock(&dev->image_mutex);
		if (init_ret < 0) {
			pr_err("v4l2-loopback: dynamic buffer initialization "
			       "failed on STREAMON\n");
			return init_ret;
		}
	}
	...
	if (!opener->buffer_count || !(opener->format_token & token)) {
		pr_err("v4l2-loopback: STREAMON with no buffers allocated "
		       "(count=%u, token=0x%x)\n",
		       opener->buffer_count,
		       opener->format_token);
		return -EINVAL;
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		if (dynamic_mode) {
			...
			if (!(opener->stream_token & token)) {
				dprintk("CAPTURE acquiring token in dynamic "
					"mode\n");
				acquire_token(dev, opener, stream, token);
				client_usage_queue_event(dev->vdev);
				pr_info("v4l2-loopback: CAPTURE stream started "
					"(dynamic mode)\n");
			}
			result = 0;
		} else {
			...
			if (has_output_token(dev->stream_tokens) &&
			    !dev->keep_format) {
				pr_warn("v4l2-loopback: CAPTURE cannot start "
					"with OUTPUT active\n");
				return -EIO;
			}
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		if (dynamic_mode) {
			...
			if (!(opener->stream_token & token)) {
				dprintk("OUTPUT acquiring token in dynamic "
					"mode\n");
				acquire_token(dev, opener, stream, token);
				...
				pr_info("v4l2-loopback: OUTPUT stream started "
					"(dynamic mode)\n");
			}
			result = 0;
		} else {
			/* Static mode: traditional validation */
			if (dev->stream_tokens & token) {
				dprintk("OUTPUT acquiring token in static "
					"mode\n");
				acquire_token(dev, opener, stream, token);
			}
			result = 0;
	...
	dprintk("STREAMON END: result=%d, stream_tokens=0x%x, "
		"opener->stream_token=0x%x\n",
		result,
		dev->stream_tokens,
		opener->stream_token);
	return result;
...
static int start_fileio(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	if (dev->use_dynamic_buffering && type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
		dprintk("start_fileio: dynamic OUTPUT mode - simplified "
			"setup\n");
...
static void free_buffers(struct v4l2_loopback_device *dev)
{
	...
	if (!has_no_owners(dev) || any_buffers_mapped(dev)) {
		printk(KERN_WARNING
		       "v4l2-loopback: buffers of video device #%u freed while "
		       "still mapped to userspace\n",
		       dev->vdev->num);
	}
			    ...
			    struct v4l2_pix_format *pix_format)
{
	...
	if (dev->image && (!has_no_owners(dev) || any_buffers_mapped(dev))) {
		if (!dynamic_mode) {
			pr_warn("v4l2-loopback: cannot reallocate buffers in "
				"use\n");
			return -EBUSY;
		}
		/* In dynamic mode, we allow more flexible relocation */
		pr_info("v4l2-loopback: reallocating buffers in dynamic "
			"mode\n");
	}
	...
	pr_info("v4l2-loopback: buffers allocated successfully (%lu bytes, "
		"dynamic=%d)\n",
		dev->image_size,
		dynamic_mode);
	return 0;
...
static void init_vdev(struct video_device *vdev, int nr)
{
	...
	vdev->device_caps = V4L2_CAP_DEVICE_CAPS | V4L2_CAP_VIDEO_CAPTURE |
		V4L2_CAP_VIDEO_OUTPUT | V4L2_CAP_READWRITE | V4L2_CAP_STREAMING;
#endif
...
static int init_dynamic_buffer(struct v4l2_loopback_device *dev)
{
	...
	if (already_initialized) {
		pr_info("v4l2-loopback: dynamic buffer already initialized, "
			"reusing\n");
		return 0;
	}
	...
	if (!dev->buffer_size) {
		if (dev->pix_format.sizeimage > 0) {
			...
			pr_info("v4l2-loopback: buffer_size derived from "
				"pix_format: %u bytes\n",
				dev->buffer_size);
		} else {
			pr_warn("v4l2-loopback: buffer_size and sizeimage not "
				"set, using default\n");
			dev->buffer_size =
	...
	if (unlikely(check_mul_overflow(dev->buffer_size,
					dev->buffer_count,
					&total_size))) {
		pr_err("v4l2-loopback: overflow in buffer calculation (%u × "
		       "%u)\n",
		       dev->buffer_size,
		       dev->buffer_count);
		return -EOVERFLOW;
	...
	if (unlikely(!dbuf)) {
		pr_err("v4l2-loopback: dynamic_buffer structure allocation "
		       "failed\n");
		return -ENOMEM;
	}
	...
	if (unlikely(!dbuf->data)) {
		...
		if (!dbuf->data) {
			pr_err("v4l2-loopback: critical buffer allocation "
			       "failure\n");
			kfree(dbuf);
			return -ENOMEM;
		}
	...
	if (unlikely(dev->dbuf)) {
		...
		vfree(dbuf->data);
		kfree(dbuf);
		pr_info("v4l2-loopback: dynamic buffer initialized by another "
			"thread\n");
	...
	pr_info("v4l2-loopback: dynamic buffer initialized successfully (%zu "
		"bytes)\n",
		initial_size);
	return 0;
}
				 ...
				 u32 copy_len)
{
	...
	if (first_chunk < copy_len) {
		memcpy((u8 *)dst_data + first_chunk,
		       src_data,
...
static int __resize_buffer_locked(struct dynamic_buffer *dbuf, u32 new_capacity)
{
	...
	if (unlikely(!new_data)) {
		pr_err("v4l2-loopback: failed to allocate %u bytes for "
		       "resize\n",
		       new_capacity);
		return -ENOMEM;
	}
	...
	if (available > 0) {
		__copy_circular_data(dbuf->data,
				     old_size,
				     dbuf->read_pos,
				     new_data,
				     available);
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr)
{
	...
	if (nr >= 0) {
		err = idr_alloc(&v4l2loopback_index_idr,
				dev,
				nr,
				nr + 1,
				GFP_KERNEL);
...
static long v4l2loopback_control_ioctl(struct file *file,
				       unsigned int cmd,
				       unsigned long parm)
{
	struct v4l2_loopback_device *dev;
...
static int __init v4l2loopback_init_module(void)
{
	...
	for (i = 0; i < devices; i++) {
		...
		cfg.card_label[0] = 0;
		if (card_label[i])
			snprintf(cfg.card_label,
				 sizeof(cfg.card_label),
				 "%s",
				 card_label[i]);
