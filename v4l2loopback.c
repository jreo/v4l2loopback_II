 ...
 *   1 = Enabled
 *
 * This parameter affects all v4l2 loopback devices when set at module load
 * time. Can be modified at runtime through sysfs at:
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * struct dynamic_buffer - Core structure for dynamic circular buffer
 * implementation
 *
...
#define acquire_token(dev, opener, label, token)                               \
  do {                                                                         \
    ...
    dprintk("acquire_token: opener=%p got token=0x%x (dynamic=%d)\n", opener,  \
            token, (dev)->use_dynamic_buffering);                              \
  } while (0)
...
static ssize_t attr_store_dynamic_buffering(struct device *cd,
  ...
  /* If Dynamic Buffering is enabled and still don't have a buffer, try
     allocating it */
 ...
 *
 * Generates a detailed human-readable report of dynamic buffer statistics
 * including:
...
static ssize_t dynamic_buffer_stats_show(struct device *cd,
  ...
  if (dbuf->stats.expand_count > 0)
    avg_expand_time_us =
        dbuf->stats.total_expand_time_us / dbuf->stats.expand_count;
  if (dbuf->stats.shrink_count > 0)
    avg_shrink_time_us =
        dbuf->stats.total_shrink_time_us / dbuf->stats.shrink_count;

  len +=
      sprintf(buf + len, "=== V4L2-Loopback Dynamic Buffer Statistics ===\n\n");
 ...
 *
 * Converts a byte size into a human-readable format with appropriate unit (B,
 * KB, MB, GB). Uses integer arithmetic to calculate decimal places for KB/MB/GB
 * conversions. Ensures the formatted string fits in the provided buffer and is
...
static void format_buffer_size(size_t size, char *buf, size_t buf_size) {
  ...
  if (unit_index == 0) {
    ...
    if (decimal_part == 0) {
      ...
      snprintf(buf, buf_size, "%zu.%02zu %s", size_copy, decimal_part,
               units[unit_index]);
    }
  }
}
...
static int vidioc_s_fmt_vid(struct file *file, void *fh,
  ...
  if (dynamic_mode) {
    ...
    dprintk(
        "S_FMT: dynamic mode - allowing format without token restriction\n");
  } else {
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * sync_dynamic_to_v4l2_buffer - Synchronizes data from dynamic buffer to V4L2
 * buffer
 ...
 *
 * Copies data from the dynamic buffer to the V4L2 buffer, handling buffer
 * management and synchronization. If dynamic buffering is disabled or data is
 * unavailable, fills the destination buffer with zeros. Manages reference
 * counting for the dynamic buffer and handles cleanup when the last reference
 * is released.
 *
...
static int get_capture_buffer(struct file *file) {
  ...
  /* === ADDED THIS BLOCK === */
    ...
    int sync_ret = sync_dynamic_to_v4l2_buffer(dev, &dev->buffers[index],
                                               dev->buffer_size);
    if (sync_ret < 0) {
...
static int vidioc_streamon(struct file *file, void *fh,
  ...
  dprintk(
      "STREAMON START: type=%d, token=0x%x, dynamic=%d, stream_tokens=0x%x\n",
  ...
  if (dev->use_dynamic_buffering && !dev->dbuf &&
    ...
    if (init_ret < 0) {
      pr_err(
          "v4l2-loopback: dynamic buffer initialization failed on STREAMON\n");
      return init_ret;
    }
  }
  ...
  if (!opener->buffer_count || !(opener->format_token & token)) {
    pr_err("v4l2-loopback: STREAMON with no buffers allocated (count=%u, "
           "token=0x%x)\n",
...
static ssize_t v4l2_loopback_read(struct file *file, char __user *buf,
  ...
  if (dev->use_dynamic_buffering && dev->dbuf) {
    ...
    ret = dynamic_buffer_read(dev, kbuf, count, !(file->f_flags & O_NONBLOCK));
    if (ret > 0) {
...
static void free_buffers(struct v4l2_loopback_device *dev) {
  ...
  if (!has_no_owners(dev) || any_buffers_mapped(dev)) {
    printk(KERN_WARNING "v4l2-loopback: buffers of video device #%u freed "
                        "while still mapped to userspace\n",
           dev->vdev->num);
  }
...
static int allocate_buffers(struct v4l2_loopback_device *dev,
  ...
  dprintk("allocate_buffers: %lu bytes (%u × %u), dynamic=%d\n", image_size,
          buffer_size, dev->buffer_count, dynamic_mode);
  ...
  pr_info(
      "v4l2-loopback: buffers allocated successfully (%lu bytes, dynamic=%d)\n",
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * calculate_buffer_used_bytes - Calculates the number of used bytes in a
 * circular buffer
 * @dbuf: Pointer to the dynamic buffer structure
 *
 * Computes the amount of data currently stored in a circular buffer by
 * comparing read and write positions. Handles both linear (write_pos >
 * read_pos) and wrapped (write_pos < read_pos) buffer states.
 *
 ...
 * @dev: Pointer to the v4l2_loopback_device structure
 *
 * Allocates and initializes a circular buffer for dynamic frame storage.
 * Handles:
 ...
 *
 * The function is idempotent and will reuse existing buffers if called multiple
 * times. Performs extensive error checking and returns appropriate error codes.
 *
...
static int init_dynamic_buffer(struct v4l2_loopback_device *dev) {
  ...
  if (!dev->buffer_size) {
    if (dev->pix_format.sizeimage > 0) {
      ...
      pr_warn(
          "v4l2-loopback: buffer_size and sizeimage not set, using default\n");
  ...
  pr_info(
      "v4l2-loopback: dynamic buffer initialized successfully (%zu bytes)\n",
      initial_size);
  return 0;
}
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * free_dynamic_buffer - Safely deallocates a dynamic buffer from V4L2 loopback
 * device
 * @dev: Pointer to the v4l2_loopback_device structure containing the buffer
 *
 * Performs a thread-safe deallocation of the dynamic buffer with proper
 * cleanup:
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * __copy_circular_data - Copies data from a circular buffer to a linear
 * destination
 ...
 *
 * Efficiently copies data from a circular buffer to a linear destination
 * buffer, handling the wrap-around case where data spans the end and beginning
 * of the circular buffer. The function performs the copy in at most two memcpy
 * operations:
 ...
 * Copyright (C) 2025 Eli Oliveira Junior
 *
 * __resize_buffer_locked - Resizes a dynamic buffer while preserving its
 * contents
...
static int __resize_buffer_locked(struct dynamic_buffer *dbuf,
  ...
  if (available > 0) {
    __copy_circular_data(dbuf->data, old_size, dbuf->read_pos, new_data,
                         available);
  }
 ...
 * @block: Whether to block waiting for data (true) or return immediately
 * (false)
 *
...
static int dynamic_buffer_read(struct v4l2_loopback_device *dev, u8 *dst,
  ...
  if (dbuf->size > dbuf->initial_size * 2 &&
    ...
    u32 target_size =
        max(dbuf->initial_size, max(available * 4, (u32)MIN_BUFFER_SIZE));
